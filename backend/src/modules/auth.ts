...
authRouter.patch("/profile",authenticate,async(req,res)=>{const body=z.object({fullName:z.string().min(2).max(100),avatarUrl:z.string().url().nullable().optional()}).parse(req.body);await prisma.user.update({where:{id:req.auth!.id},data:{fullName:body.fullName,avatarUrl:body.avatarUrl ?? null}});res.json({data:await getAppUser(req.auth!.id)});});
