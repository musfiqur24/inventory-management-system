...
authRouter.patch("/profile",authenticate,async(req,res)=>{const body=z.object({fullName:z.string().min(2).max(100),avatarUrl:z.string().url().nullable().optional(),phone:z.string().trim().max(30).nullable().optional(),presentAddress:z.string().trim().max(500).nullable().optional(),permanentAddress:z.string().trim().max(500).nullable().optional()}).parse(req.body);await prisma.user.update({where:{id:req.auth!.id},data:{fullName:body.fullName,avatarUrl:body.avatarUrl ?? null,phone:body.phone || null,presentAddress:body.presentAddress || null,permanentAddress:body.permanentAddress || null}});res.json({data:await getAppUser(req.auth!.id)});});
