...
rmStoreRouter.post('/receive', async (req, res) => {
  ...
  const result = await prisma.$transaction(async tx => {
    ...
    if (!['APPROVED','INCOMPLETE','PARTIALLY_RECEIVED'].includes(delivery.status)) throw new StockError('DELIVERY_UNAVAILABLE','Only a manager-verified challan can be received.');
    ...
    for (const alloc of allocations) {
      ...
      if (line.verifiedQty == null || qty.plus(line.acceptedQty ?? 0).gt(line.verifiedQty)) throw new StockError('OVER_RECEIPT','Received quantity exceeds the manager-verified quantity.');
    ...
    await tx.supplierDelivery.update({where:{id:delivery.id},data:{status:allLines.every(l=>new Prisma.Decimal(l.acceptedQty??0).gte(l.verifiedQty??0))?DocumentStatus.RECEIVED:DocumentStatus.PARTIALLY_RECEIVED}});
    if (requisition) {
