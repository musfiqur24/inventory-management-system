...
partnersRouter.post("/", async (req, res) => {
  ...
  const partner = await prisma.partner.create({
    data: { ...parsed, organizationId: req.tenantId! },
  });
  res.status(201).json({ data: partner });
});
const updatePartner: RequestHandler = async (req, res) => {
  const parsed = (
    req.method === "PATCH" ? updatePartnerSchema : createPartnerSchema
  ).parse(req.body);
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (
      parsed.partnerType &&
      parsed.partnerType !== existing.partnerType &&
      (await isReferenced(tx, existing.id))
    )
  ...
  if (result.kind === "missing") {
    res
      .status(404)
      .json({ error: { code: "NOT_FOUND", message: "Partner not found" } });
    return;
  }
  if (result.kind === "in-use") {
    res
      ...
      .json({
        error: {
          code: "PARTNER_IN_USE",
          message: "The type of a partner used in documents cannot be changed.",
        },
      });
    return;
  }
    ...
    return res
      ...
      .json({
        error: {
          code: "PARTNER_IN_USE",
          message:
            "This partner is used in requisitions, deliveries, or sales orders and cannot be deleted.",
        },
      });
