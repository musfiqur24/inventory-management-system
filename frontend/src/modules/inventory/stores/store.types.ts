...
export interface StockBalance { id: string; productId: string; requisitionNumber?: string | null; lotId: string; binId: string; uomId: string; quantity: string | number; reservedQty: string | number; updatedAt: string; product: {name:string;sku:string}; lot: {code:string;expiryDate?:string;qualityStatus:string}; bin: Bin; uom: {code:string}; }
