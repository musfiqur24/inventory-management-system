...
interface Position {
  ...
  quantity: number;
  reservedQty: number;
  lot?: { code?: string; qualityStatus?: string; expiryDate?: string | null };
  bin: {
    code: string;
    name: string;
    storeId?: string;
...
export function MaterialIssuesPage() {
  ...
  const [allocationShortfalls, setAllocationShortfalls] = useState<
    AllocationShortfall[]
  >([]);
  ...
  useEffect(() => {
    if (open)
      void api<{ data: Position[] }>("/rm-store/balances")
  ...
  const allocateOrder = (
    ...
    storeId: string,
    preferred = preferredPositions,
  ) => {
    if (!productionOrderId || !storeId) {
      setLines([]);
    ...
    const availableByPosition = new Map(
      positions.map((position) => [
        position.id,
        Math.max(0, Number(position.quantity) - Number(position.reservedQty)),
      ]),
    );
    ...
    for (const requirement of order.lines ?? []) {
      let remaining = Math.max(
        0,
        Number(requirement.requiredQty) - Number(requirement.issuedQty),
      );
      if (remaining <= 0) continue;
      const candidates = positions
        .filter((position) => {
          const selectedStore =
            (position.bin.storeId ?? position.bin.store?.id) === storeId;
          const released =
            !position.lot?.qualityStatus ||
            position.lot.qualityStatus === "RELEASED";
          const unexpired =
            !position.lot?.expiryDate ||
            new Date(position.lot.expiryDate).getTime() >= Date.now();
          return (
            selectedStore &&
            position.productId === requirement.productId &&
            position.uomId === requirement.uomId &&
            released &&
            unexpired &&
            (availableByPosition.get(position.id) ?? 0) > 0
          );
        })
        .sort((a, b) => {
          const preference =
            preferred[requirement.productId + ":" + requirement.uomId];
          if (a.id === preference && b.id !== preference) return -1;
          if (b.id === preference && a.id !== preference) return 1;
          const first = a.lot?.expiryDate
            ? new Date(a.lot.expiryDate).getTime()
            : Number.MAX_SAFE_INTEGER;
          const second = b.lot?.expiryDate
            ? new Date(b.lot.expiryDate).getTime()
            : Number.MAX_SAFE_INTEGER;
          return first - second;
        });
      for (const position of candidates) {
        ...
        generated.push({
          rawMaterialId: requirement.productId,
          lotId: position.lotId,
          fromBinId: position.binId,
          uomId: requirement.uomId,
          issuedQty: String(Number(allocated.toFixed(6))),
        });
      ...
      if (remaining > 0.000001)
        shortfalls.push({
          name:
            requirement.rawMaterial?.name ??
            products.find((p) => p.id === requirement.productId)?.name ??
            "Raw material",
  ...
  useEffect(() => {
    if (open && form.productionOrderId && form.storeId)
      allocateOrder(form.productionOrderId, form.storeId);
  ...
  const submit = async () => {
    ...
    if (
      ...
      return setMessage("Review the allocation and complete every issue line.");
    setSaving(true);
    try {
      const validLines = lines.filter(
        (l) => l.rawMaterialId && l.uomId && l.issuedQty,
      );
      ...
      setOpen(false);
      setForm({ productionOrderId: "", storeId: "" });
      setLines([]);
      setAllocationShortfalls([]);
      void load();
    } catch (e: any) {
  ...
  const allocationReview = (() => {
    ...
    for (const line of lines) {
      ...
      const position = positions.find(
        (item) =>
          item.binId === line.fromBinId &&
          item.lotId === line.lotId &&
          item.productId === line.rawMaterialId,
      );
      const source =
        (position?.lot?.code ??
          lots.find((lot) => lot.id === line.lotId)?.lotNumber ??
          "Lot") +
        " / " +
        (position?.bin.code ?? "Bin") +
        " - " +
        Number(line.issuedQty).toLocaleString();
      current.sources.push(source);
    }
    return [...grouped.values()].map((item) => ({
      ...item,
      insufficient: Math.max(0, item.required - item.allocated),
      uom: uoms.find((uom) => uom.id === item.uomId)?.code ?? "",
    }));
  })();
        ...
        <div className="overflow-x-auto">
          ...
          >
            {rows.map((mi) => (
              <tr key={mi.id}>
                <td>
                  <strong className="text-[#0d3b2e] font-mono">
                    {mi.issueNumber}
                  </strong>
                </td>
                <td>
      ...
      {open && (
        ...
        >
          <div className="grid grid-cols-2 gap-3.5 max-[700px]:grid-cols-1">
            ...
            <FormField label="RM Store" required>
              <Dropdown
                value={form.storeId}
                onChange={(e) =>
                  setForm((current) => ({
              ...
              >
                <option value="">Select RM store</option>
                {stores.map((store) => (
          ...
          <div className="[border-top:1px_solid_#e0e5dd] pt-3.5">
            <div className="mb-2.5 flex items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-0.5 text-[12px] text-[#7a9185]">
                  Available stock is allocated by earliest expiry. Stock is
                  deducted only after confirmation.
                </div>
              </div>
                ...
                <div className="min-w-[920px]">
                  ...
                  {allocationReview.map((item) => {
                    ...
                    const eligiblePositions = positions
                      .filter((position) => {
                        const selectedStore =
                          (position.bin.storeId ?? position.bin.store?.id) ===
                          form.storeId;
                        const released =
                          !position.lot?.qualityStatus ||
                          position.lot.qualityStatus === "RELEASED";
                        const unexpired =
                          !position.lot?.expiryDate ||
                          new Date(position.lot.expiryDate).getTime() >=
                            Date.now();
                        return (
                          selectedStore &&
                          position.productId === item.productId &&
                          position.uomId === item.uomId &&
                          released &&
                          unexpired &&
                          Number(position.quantity) -
                            Number(position.reservedQty) >
                            0
                        );
                      })
                      .sort((a, b) => a.bin.code.localeCompare(b.bin.code));
                    const firstAllocation = lines.find(
                      (line) =>
                        line.rawMaterialId === item.productId &&
                        line.uomId === item.uomId,
                    );
                    const selectedPosition = eligiblePositions.find(
                      (position) =>
                        position.binId === firstAllocation?.fromBinId &&
                        position.lotId === firstAllocation?.lotId,
                    );
                    return (
                      <div
                        key={item.productId + item.uomId}
                        className="grid grid-cols-[minmax(190px,1.35fr)_110px_110px_125px_minmax(250px,1.8fr)_105px] items-center gap-3 border-t border-[#e0e5dd] px-4 py-3 text-[13px]"
                      >
                        <div className="min-w-0">
                          <strong className="block truncate text-[#173b30]">
                            {item.name}
                          </strong>
                          {item.sku && (
                            <span className="block truncate text-[11px] text-[#7a9185]">
                              {item.sku}
                            </span>
                          )}
                        </div>
                        <strong>
                          {item.required.toLocaleString()} {item.uom}
                        </strong>
                        <span
                          className={
                            item.allocated > 0
                              ? "font-semibold text-[#176b4b]"
                              : "font-semibold text-[#9a3b32]"
                          }
                        >
                          {item.allocated.toLocaleString()} {item.uom}
                        </span>
                        <strong
                          className={
                            shortage ? "text-[#c34838]" : "text-[#176b4b]"
                          }
                        >
                          {shortage
                            ? item.insufficient.toLocaleString() +
                              " " +
                              item.uom
                            : "0 " + item.uom}
                        </strong>
                        <div className="min-w-0">
                          <Dropdown
                            aria-label={"Allocation bin for " + item.name}
                            value={
                              preferredPositions[
                            ...
                            disabled={!eligiblePositions.length}
                            onChange={(event) => {
                              const next = {
                                ...preferredPositions,
                                [item.productId + ":" + item.uomId]:
                          ...
                          >
                            <option value="">
                              {eligiblePositions.length
                                ? "Select allocation bin"
                                : "Not available in selected store"}
                            </option>
                            {eligiblePositions.map((position) => (
                              <option key={position.id} value={position.id}>
                                {position.bin.name || position.bin.code}
                              </option>
                            ))}
                          </Dropdown>
                          {item.sources.length > 1 && (
                            <span className="mt-1 block text-[10.5px] text-[#7a9185]">
                              Allocation continues across {item.sources.length}{" "}
                              bins when required.
                            </span>
                          )}
                        </div>
                        <span
                          className={
                            shortage
                              ? "w-fit rounded-full bg-[#fde8e5] px-2.5 py-1 text-[11px] font-semibold text-[#b23b31]"
                              : "w-fit rounded-full bg-[#e5f5eb] px-2.5 py-1 text-[11px] font-semibold text-[#16734f]"
                          }
                        >
                          {shortage
                            ? item.allocated > 0
