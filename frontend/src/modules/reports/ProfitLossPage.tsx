...
export function ProfitLossPage() {
  ...
  return <PageContainer loading={loading} cap="REPORTS" title="Profit & Loss" description="Batch profitability calculated from Product setup amounts and posted sales prices.">
    <div className="summary-grid mb-5 grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4"><Card className="p-5"><WalletCards className="mb-3 text-[#7a9185]"/><span className="text-xs text-[#7a9185]">Production cost</span><b className="block text-xl">{money(cost)}</b></Card><Card className="p-5"><TrendingUp className="mb-3 text-[#1b8f5a]"/><span className="text-xs text-[#7a9185]">Revenue</span><b className="block text-xl">{money(revenue)}</b></Card><Card className="p-5">{profit >= 0 ? <TrendingUp className="mb-3 text-[#1b8f5a]"/> : <TrendingDown className="mb-3 text-red-600"/>}<span className="text-xs text-[#7a9185]">Net profit / loss</span><b className={`block text-xl ${profit < 0 ? 'text-red-600' : 'text-[#1b8f5a]'}`}>{money(profit)}</b></Card></div>
