...
export function LoginPage(){
 ...
 const [email,setEmail]=useState("");const [password,setPassword]=useState("");const [showPassword,setShowPassword]=useState(false);const [submitting,setSubmitting]=useState(false);const [forgotSubmitting,setForgotSubmitting]=useState(false);
 ...
 const requestPasswordReset=async()=>{if(!email.trim()){appToast.error("Enter your email address first.");return;}setForgotSubmitting(true);try{await api("/auth/forgot-password",{method:"POST",body:JSON.stringify({email})});appToast.success("If an active account matches this email, a reset link has been sent.");}catch(reason){appToast.error(reason instanceof Error?reason.message:"Unable to request a reset link.");}finally{setForgotSubmitting(false);}};
 return <main className="grid min-h-dvh place-items-center overflow-hidden bg-[radial-gradient(circle_at_top_left,#e7efe5_0,transparent_38%),#f3f5f2] p-3 sm:p-5">
  <section className="grid w-full max-w-285 overflow-hidden rounded-3xl border border-[#d7e0d8] bg-white shadow-[0_28px_80px_rgba(13,59,46,0.14)] lg:min-h-170 lg:grid-cols-[1.05fr_.95fr]">
   <aside className="relative hidden overflow-hidden bg-[#0d3b2e] p-10 text-white lg:flex lg:flex-col xl:p-12">
    ...
    <div className="relative my-auto mx-auto max-w-110 text-center"><h1 className="font-['Outfit',sans-serif] text-5xl font-bold leading-[1.06] tracking-[-.035em] xl:text-[56px]">Inventory decisions, under control.</h1><p className="mx-auto mt-6 max-w-95 text-[15px] leading-7 text-white/60">One secure workspace for procurement, production, inventory, and traceability.</p></div>
   ...
   <div className="flex items-center justify-center px-5 py-8 sm:px-10 sm:py-12">
    <form onSubmit={submit} className="w-full max-w-105">
     <div className="mb-8 flex items-center gap-3 lg:hidden"><div className="size-14 overflow-hidden rounded-xl bg-white p-1 shadow-sm ring-1 ring-[#dce3dd]"><img src="/Inventory_Logo.png" alt="Feed Track logo" className="size-full object-contain"/></div><div><strong className="block font-['Outfit',sans-serif] text-xl">Feed Track</strong></div></div>
     <div className="mb-8 hidden flex-col items-center text-center lg:flex"><img src="/Inventory_Logo.png" alt="Feed Track logo" className="size-28 object-contain"/><strong className="mt-3 block font-['Outfit',sans-serif] text-4xl font-bold tracking-[-.025em] text-[#0f1c16]">Feed Track</strong></div>
     <div className="mb-8 text-center"><h2 className="font-['Outfit',sans-serif] text-3xl font-bold tracking-[-.025em] text-[#0f1c16]">Welcome back</h2><p className="mt-2 text-sm leading-6 text-[#74887d]">Sign in to continue to your assigned workspace.</p></div>
     ...
     <button type="button" onClick={requestPasswordReset} disabled={forgotSubmitting} className="mb-5 ml-auto block text-sm font-semibold text-[#1a5c45] hover:underline disabled:cursor-not-allowed disabled:opacity-50">{forgotSubmitting?"Sending reset link...":"Forgot password?"}</button>
     <Button type="submit" variant="primary" disabled={submitting||forgotSubmitting} className="min-h-12 w-full text-sm">{submitting?"Signing in...":<>Sign in <ArrowRight size={18}/></>}</Button>
    </form>
   </div>
  </section>
 </main>;
