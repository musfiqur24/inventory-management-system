...
export function PageHeader({title,description,actions,cap,content}:Props){
  return <header className="mb-4 rounded-xl border border-[#dfe5df] bg-white px-5 py-5 shadow-[0_1px_2px_rgba(15,28,22,0.04)] sm:mb-5 sm:px-6">
    ...
    {actions&&<div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:shrink-0 [&>button]:max-sm:flex-1">{actions}</div>}</div>
    {content&&<div className="mt-5 border-t border-[#e0e5dd] pt-5">{content}</div>}
  </header>;
}
