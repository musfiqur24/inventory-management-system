...
export function Modal({title,description,children,footer,wide,extraWide,fixedHeight,onClose}:Props){
 ...
 return <div className="fixed inset-0 z-100 flex items-end justify-center bg-[#0c1812]/65 p-0 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}>
  <div role="dialog" aria-modal="true" aria-labelledby={titleId} className={twMerge("flex max-h-[94dvh] w-full max-w-150 flex-col overflow-hidden rounded-t-2xl border border-[#dce3dd] bg-white shadow-[0_24px_64px_rgba(0,0,0,0.25)] sm:max-h-[90dvh] sm:rounded-2xl",wide&&"max-w-210",extraWide&&"max-w-250",fixedHeight&&"h-[min(42rem,90dvh)]")}>
   ...
   <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-5 sm:px-7 sm:py-6">{children}</div>
   {footer&&<footer className="flex shrink-0 flex-col-reverse gap-2 border-t border-[#e3e8e3] bg-[#f8faf8] px-5 py-4 sm:flex-row sm:justify-end sm:px-7 [&>button]:max-sm:w-full">{footer}</footer>}
  </div>
 </div>;
}
