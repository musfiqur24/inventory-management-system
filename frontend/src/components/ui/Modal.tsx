...
export function Modal({title,description,children,footer,extraWide,fixedHeight,onClose}:Props){
 const titleId=useId();
 ...
 return <div className="fixed inset-0 z-100 flex items-end justify-center bg-[#07130e]/72 p-0 backdrop-blur-[6px] sm:items-center sm:p-6" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}>
  <div role="dialog" aria-modal="true" aria-labelledby={titleId} className={twMerge("relative flex max-h-[95dvh] w-full max-w-210 flex-col overflow-hidden rounded-t-md border border-white/70 bg-[#fffdf8] shadow-[0_32px_90px_rgba(5,20,14,0.34),0_8px_24px_rgba(5,20,14,0.16)] ring-1 ring-[#193b2d]/10 sm:max-h-[90dvh] sm:rounded-md",extraWide&&"max-w-[1328px]",fixedHeight&&"h-[min(49rem,90dvh)]")}>
