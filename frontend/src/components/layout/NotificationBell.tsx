...
type Notification={id:string;title:string;message:string;requisitionId:string;createdAt:string;readAt:string|null};
export function NotificationBell(){
 ...
 useEffect(()=>{
  let active=true;
  const load=()=>api<{data:Notification[];unreadCount:number}>('/notifications').then(r=>{if(active){setItems(r.data);setCount(r.unreadCount);setError('')}}).catch(()=>{if(active)setError('Unable to load notifications.');});
